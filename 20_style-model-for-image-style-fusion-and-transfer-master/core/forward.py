...
def forward(inputs, weight):
    ...
    # 新增2层resBlock
    # inputs = res_block("res6", inputs, 3, 128, 128, weight)
    # inputs = res_block("res7", inputs, 3, 128, 128, weight)
...
def conditional_normalization(x, scope_bn, weight=None):
    ...
    # 在test过程当中weight可以融合多个风格的参数，tf.从而达到特征融合效果（具体可参考上述论文）
...
def upsampling(name, inputs, nums_in, nums_out, weight):
    ...
    # 双三次插值算法
    inputs = tf.image.resize_bicubic(inputs, [tf.shape(inputs)[1] * 2, tf.shape(inputs)[2] * 2])
    # 卷积
